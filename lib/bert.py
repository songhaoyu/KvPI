...
class BERTLM(nn.Module):
    def __init__(self, local_rank, vocab, embed_dim, ff_embed_dim, num_heads, dropout, layers, approx):
        ...
        self.typ_embed = Embedding(4, embed_dim, None)
    ...
    def work(self, inp, seg=None, typ=None, layers=None):
        ...
        # type (torch.Tensor): segment ids, size: (seq_len x bsz), default is None, which means all zeros.
        ...
        x = self.tok_embed(inp) + self.pos_embed(inp) + self.seg_embed(seg) + self.typ_embed(typ)
